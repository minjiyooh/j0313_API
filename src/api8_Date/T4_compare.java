...
//		String bigyoToday = strTempToday.substring(0,strTempToday.indexOf("."));
		...
		if (strDate.equals(bigyoDate))
			System.out.println("같은날");
		else
			System.out.println("다른날");

		// 날짜를 날짜형식으로 비교하고자 한다면?
		// isEqual() : 동일날짜비교, 이전날짜비교 : isBefore(), 이후날짜비교: isAfter(),날짜 차이 : period
		...
		Period period = Period.between(starDate, targetDate);
...
//		System.out.println(lastDay + "달의 마지막 일자? " + YearMonth.from(bigyoDate2).atEndOfMonth());
		String lastDay2 = "2024-03-2";
		System.out.println(lastDay2 + "달의 마지막 일자? " + YearMonth.from(LocalDate.parse(lastDay2,DateTimeFormatter.ofPattern("yyyy-MM-d"))).atEndOfMonth());
	}
}
